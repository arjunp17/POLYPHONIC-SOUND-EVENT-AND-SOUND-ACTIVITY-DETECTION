...
## baseline model
def baselineSAD(rows,cols,channels,nb_classes,lr):
	...
	sad_out = TimeDistributed(Dense(1, activation='sigmoid', name = 'sad_out'))(dense)
	
	model = Model(input, sad_out, name='basemodelSAD')	
	opt = Adam(lr = lr)
...
baselineSAD = baselineSAD(rows,cols,channels,nb_classes,lr)


# feature and label
X_train = np.load('../train_feature.npy')
Y_train = np.load('../train_label_SAD.npy')
X_val = np.load('../val_feature.npy')
Y_val = np.load('../val_label_SAD.npy')
...
train_history = model_train(baselineSAD,X_train,Y_train,X_val,Y_val,epochs,batch_size,SAD,output_folder)
